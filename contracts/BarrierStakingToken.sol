...
contract BARRIER is MintableToken, HasNoEther 
{
    // Using libraries 
    ...
    */
    ...
    string public constant name = "BARRIER PROTOCOL";
    string public constant symbol = "BARRIER";
    ...
    uint public totalBARRIERStaked; 
    ...
    /// @dev BARRIER token constructor
    function BARRIERTOKEN() public
    {
        // Define owner
        owner = msg.sender;
        // Define initial owner supply. (ether here is used only to get the decimals right)
        uint _initOwnerSupply = 400000 ether;
        ...
        // 300 minted tokens per day, 86,400 seconds in a day
        ownerMintRate = calculateFraction(300, 86400, decimals);
        
        // 110,000 targeted minted tokens per year via staking; 31,536,000 seconds per year
        globalMintRate = calculateFraction(110000, 31536000, decimals);
    }

    /// @dev staking function which allows users to stake an amount of tokens to gain interest for up to 10 days 
    function stakeBARRIER(uint _stakeAmount) external
    {
    ...
    function stakeBARRIERSplit(uint _stakeAmount, address _stakeSplitAddress) external
    {
        // Require that a BARRIER split actually be split with an address
    ...
    function claimStake() external returns (bool success)
    {
        ...
        // Subtract stake balance from totalBARRIERStaked
        totalBARRIERStaked -= stakeBalances[msg.sender].initialStakeBalance;
    ...
    function stakeTokens(uint256 _value) private returns (bool success)
    {
        ...
        totalBARRIERStaked += _value;

        /// Determine percentage of global stake
        stakeBalances[msg.sender].initialStakePercentage = calculateFraction(_value, totalBARRIERStaked, decimals);
    ...
    function calculateStakeGains(uint _timePassedSinceStake) view private returns (uint mintTotal)
    {
        ...
        uint _finalMintRate;            // store our calculated final mint rate (in BARRIER-per-second)
        ...
        if (_timePassedSinceStake > _secondsPerDay) {
            ...
            // First, calculate our final stake percentage based upon the total amount of BARRIER staked
            _finalStakePercentage = calculateFraction(stakeBalances[msg.sender].initialStakeBalance, totalBARRIERStaked, decimals);
            ...
            // Finally, calculate our final mint rate (in BARRIER-per-second)
